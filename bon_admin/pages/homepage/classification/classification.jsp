...
<body>
		...
		<div class="inner-page-main-container">
			<div class="layer" >
				<div id="tableMain" style=" margin-left: 20px; margin-right: 20px">
				    <button class="" id="addFirstMenu">添加一级分类</button></br>
	...
	<script>
		...
		function addDiyDom(treeId, treeNode) {
			...
			var editStr = '';
			//editStr += '<div class="diy">' + (treeNode.menu_url == null ? '&nbsp;' : showColumnValue(treeNode.menu_url,15) ) + '</div>';
			editStr += '<div class="diy">' + (treeNode.sort == null ? '&nbsp;' : treeNode.sort ) + '</div>';
			editStr += '<div class="diy">' + (treeNode.label_name == null ? '&nbsp;' : showColumnValue(treeNode.label_name ,15) ) + '</div>';
		...
		function query() {
			...
			var datas;
			$.ajax({
				url:getRootPath()+"/classification/queryMenuList.action",
				dataType:"json",
				type:"post",
				async:false,
		...
		function formatHandle(treeNode) {
			...
			htmlStr += '<div class="icon_div"><a href="javascript:" class="update" data-id='
				+ treeNode.id +' data-pId='+treeNode.pId+'>修改</a></div>';
			htmlStr += '<div class="icon_div"><a href="javascript:" class="delete" data-id='
				+ treeNode.id +' data-pId='+treeNode.pId+'>删除</a></div>';
				
				console.log(treeNode);
				if(treeNode.pId==null)
				{	htmlStr += '<div class="icon_div"><a href="javascript:" class="add" data-id="'
				+ treeNode.id +'" data-pId="'+treeNode.pId+'" data-final="'+treeNode.final_node+'">添加二级分类 </a></div>';
				}else{
		 ...
		 layui.use(['form','element','layer'], function(){
			 ...
			 $("#addFirstMenu").click(function(){
				 window.location.href=getRootPath()+'/pages/homepage/classification/addMenu.jsp?id=0&type=add&parentID=0';
			});
			$(".ztree").on("click",".update",function(){
				
				var id=$(this).attr("data-id");
				var parentID=$(this).attr("data-pId")=="null"?"0":$(this).attr("data-pId");
				
				window.location.href=getRootPath()+'/pages/homepage/classification/editMenu.jsp?id='+id+'&type=update&parentID='+parentID;
			})
			$(".ztree").on("click",".add",function(){
				...
				{
					//非最终子节点
					window.location.href=getRootPath()+'/pages/homepage/classification/editMenu.jsp?id='+id +'&type=add&parentID='+parentID ;
			...
			$(".ztree").on("click",".delete",function(){
				...
				parent.layer.confirm('确定要删除这条信息吗？', {
					  ...
					  //按钮【按钮一】的回调
						$.ajax({
					        url : getRootPath()+"/classification/deleteMenu.action",
					        type : "post",
					        data : { id : id },
					        datatype : "text",
					        success : function (data) {
