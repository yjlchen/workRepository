			...
			layui.use([ 'form', 'layedit', 'laydate', 'jquery', 'laypage',
							...
							function() {
								...
								form.on('select(group)', function(data) {
									...
									//下拉列表被选中后实现上传
									layui.upload({
										url : getRootPath() + '/document/uploadDoc.action?group_ids='+data.value+'&type=voice'
										,ext: 'mp3|ogg|wav'
													...
													if ("success" == result) {
														parent.layer.msg('添加成功！', {
															  ...
															  time: 1000 //（如果不配置，默认是3秒）
																...
																parent.layer.closeAll();
																window.location.href = getRootPath()+"/pages/webstore/mydoc/myDocument.jsp?type=1";
															});
													} else {
...
//										$('.delgroup').attr("class","layui-btn layui-btn-disabled delgroup");
						        ...
						        var form = layui.form();
								layui.upload({
									url : getRootPath() + '/document/uploadDoc.action?group_ids='+""+'&type=voice'
									,ext: 'mp3|ogg|wav'
									,before : function(input) {
 ...
 */
						function append_table(result,group_ids) {
							...
							$.each(result,function(i, item) {
								str += "<div style='height:250px;'id='splice'>";
								str += "<div>";
								str += "<audio controls='controls'><source src='"+ item.file_url+ "'>Your browser does not support the audio element.</audio>";
								str += "</div>";
								str += "<div>";
 ...
 */
						function deleteGroup(file_group_name,id){
							parent.layer.confirm('确定要删除分组吗？仅删除分组，不删除语音，组内语音将自动归入未分组', {
								  btn: ['是', '否'] //可以无限个按钮
								}, function(index, layero){
								  //按钮【按钮一】的回调
									$.ajax({
