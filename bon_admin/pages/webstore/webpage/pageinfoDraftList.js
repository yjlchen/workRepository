...
layui.use('element', function(){ 
	  ...
	  $("#addWebpage").click(function(){
		  ...
		  parent.document.getElementById(data_pjax).src=getRootPath()+"/pages/webstore/webpage/editPageinfoDraft.jsp"
	  });
...
function searchPage(){
	...
	pageinfo = $('#pageinfo_list').DataTable({
	        ...
	        "columnDefs": [
							...
							},
							{
							    ...
							    "render": function(data, type, full) {
							    	...
							    	if(full.micro_main==1){
								    	...
								    	//+"|<a href='#' onclick='copyPageInfo(\""+id+"\")'>复制</a>
	                     ...
	                     "ajax": {
	                    	 "type":"post",
	                         "url": getRootPath()+"/webstore/pageinfo/queryPageInfoDraftList.action",
	                         "dataType":"json",
...
function updatePageInfo(id){
	var data_pjax=$("#admui-siteConTabs ul>li.active>span",window.parent.document).attr("data-pjax");
	parent.document.getElementById(data_pjax).src=getRootPath()+"/pages/webstore/webpage/updatePageinfoDraft.jsp?id="+id 
}
...
function showUrl(e,micro_url){
	var x=e.pageX;
	var y=e.pageY;
	var url = getRootPath()+'/pages/webstore/webpage/showUrl_pageInfoDraft.jsp?micro_url='+micro_url;
	layer.open({
		title: ''
        ,type: 2
        ,closeBtn: 0
