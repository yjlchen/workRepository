(function($,window,document,undefined){
	var Inits = function(ele,opts){
		...
		this.defaults = {
			_width: 320, //设置图轮播图大小
			_height: 320,
			page: 'swp-thumbnail-list', //是否启用导航图标,有值时启用，值为导航图标列表的样式类，为空时不启用导航列表
	...
	Inits.prototype = {
		slideFade: function(){
			...
			if (init.page!=''&&init.page!=undefined) {
				...
				pageli.eq(0).addClass('swiper-active-switch');
				page.css({
					position: 'absolute',
					width: pageliw+'px'
				});
...
//				};
			...
			var next = function(fade){
				...
				page.children('li').eq(i).removeClass('swiper-active-switch').next().addClass('swiper-active-switch');
				i++;
				if (i>li.length-1) {
					i=0;
					li.eq(i).fadeIn(fade);
					page.children('li').eq(i).addClass('swiper-active-switch');
				}
			};
			var prev = function(fade){
				...
				if (i==0) {
					...
					li.eq(i).fadeIn(fade);
					page.children('li').eq(0).removeClass('swiper-active-switch');
					page.children('li').eq(i).addClass('swiper-active-switch');
				}else{
					li.eq(i).fadeOut(fade).prev().fadeIn(fade);
					page.children('li').eq(i).removeClass('swiper-active-switch').prev().addClass('swiper-active-switch');
					i--;
				}
			}
			//下一张
			...
			if (init.page!=''&&init.page!=undefined) {
				...
				pageli.each(function(index, el) {
					$(this).click(function(event) {
						...
						page.children('li').eq(i).addClass('swiper-active-switch').siblings('li').removeClass('swiper-active-switch');
					});		
				});
			};
		}//slideFade end
