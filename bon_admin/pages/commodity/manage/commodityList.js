...
function initEvent(form){
	 $("#to_create").click(function(){
		 ...
		 parent.document.getElementById(data_pjax).src=getRootPath()+"/pages/commodity/manage/commodityCreate.jsp"
	});
	 ...
	 group = $('#group_list').DataTable({
	        ...
	        "columnDefs": [
							...
							},
							{
								...
								"data":"id",
								"render": function(data, type, full, meta) {
								return '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" onclick=upCommodityInfo("'+full.id+'") >编辑</a>'
								       +'-<a href="#" onclick=del_commodity("'+full.id+'") >删除</a>'
									  /* +'-<a href="#" onclick=promotion(\"'+full.id+'\",\"'+full.commodity_url+'\",event) >推广商品</a>'*/
								       /*+'-<a href="#" onclick=toCopy(\"'+full.id+'\") >复制</a>';*/
								       +'-<a href="#" onclick=batchManage(\"'+full.id+'\") >批次管理</a>';
								}
							}
	                     ],
	                     //内容居中显示	 	                     
...
function upCommodityInfo(id){
	 var data_pjax=$("#admui-siteConTabs ul>li.active>span",window.parent.document).attr("data-pjax");
	 parent.document.getElementById(data_pjax).src=getRootPath()+"/pages/commodity/manage/commodityCreate.jsp?id="+id
}
...
//	 parent.document.getElementById(data_pjax).src=getRootPath()+"/pages/commodity/manage/batchManage.jsp?commId="+id
	 location.href = getRootPath()+"/pages/commodity/manage/batchManage.jsp?commId="+id
}
