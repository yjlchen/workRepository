...
(function() {
    ...
    SelectParser = function() {
        ...
        SelectParser.prototype.add_group = function(group) {
            ...
            _results = [];
            for (_i = 0, _len = _ref.length; _i < _len; _i++) option = _ref[_i],
    ...
    SelectParser.select_to_array = function(select) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) child = _ref[_i],
...
function() {
    ...
    AbstractChosen = function() {
        ...
        AbstractChosen.prototype.set_default_text = function() {
           ...
           */
            return this.form_field.getAttribute("data-placeholder") ? this.default_text = this.form_field.getAttribute("data-placeholder")
            	: this.is_multiple ? this.default_text = this.options.placeholder_text_multiple 
            ...
            this.results_none_found = 
            	this.form_field.getAttribute("data-no_results_text") || this.options.no_results_text || "木有找到这个："
        },
        ...
        AbstractChosen.prototype.generate_field_id = function() {
            var new_id;
            return new_id = this.generate_random_id(),
            this.form_field.id = new_id,
            new_id
        },
        AbstractChosen.prototype.generate_random_char = function() {
            var chars, newchar, rand;
            return chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ",
...
//            return ! $.browser.msie || $.browser.version !== "6.0" && $.browser.version !== "7.0" ? this.each(function(input_field) {
              return ! $.browser || $.browser.version !== "6.0" && $.browser.version !== "7.0" ? this.each(function(input_field) {
                var $this;
                $this = $(this);
    ...
    Chosen = function(_super) {
        ...
        return __extends(Chosen, _super),
        ...
        Chosen.prototype.container_mousedown = function(evt) {
            ...
            if (!this.is_disabled) return target_closelink = evt != null ? $(evt.target).hasClass("search-choice-close") : !1,
            evt && evt.type === "mousedown" && !this.results_showing && evt.stopPropagation(),
            !this.pending_destroy_click && !target_closelink ? (this.active_field ? !this.is_multiple && evt && ($(evt.target)[0] === this.selected_item[0] || $(evt.target).parents("a.chzn-single").length) && (evt.preventDefault(), this.results_toggle()) : (this.is_multiple && this.search_field.val(""), $(document).click(this.click_test_action), this.results_show()), this.activate_field()) : this.pending_destroy_click = !1
        },
        ...
        Chosen.prototype.close_field = function() {
            return $(document).unbind("click", this.click_test_action),
        ...
        Chosen.prototype.results_build = function() {
            ...
            for (_i = 0, _len = _ref.length; _i < _len; _i++) data = _ref[_i],
            data.group ? content += this.result_add_group(data) : data.empty || (content += this.result_add_option(data), data.selected && this.is_multiple ? this.choice_build(data) : data.selected && !this.is_multiple && (this.selected_item.removeClass("chzn-default").find("span").text(data.text), this.allow_single_deselect && this.single_deselect_control_build()));
        ...
        Chosen.prototype.results_show = function() {
            ...
            !1;
            return dd_top = this.is_multiple ? this.container.height() : this.container.height() - 1,
        ...
        Chosen.prototype.set_tab_index = function(el) {
            var ti;
            if (this.form_field_jq.attr("tabindex")) return ti = this.form_field_jq.attr("tabindex"),
            this.form_field_jq.attr("tabindex", -1),
            this.is_multiple ? this.search_field.attr("tabindex", ti) : (this.selected_item.attr("tabindex", ti), this.search_field.attr("tabindex", -1))
        },
        ...
        Chosen.prototype.search_results_mouseup = function(evt) {
            ...
            if (target.length) return this.result_highlight = target,
        ...
        Chosen.prototype.choice_destroy_link_click = function(evt) {
            return evt.preventDefault(),
            this.is_disabled ? evt.stopPropagation: (this.pending_destroy_click = !0, this.choice_destroy($(evt.target)))
        },
        ...
        Chosen.prototype.result_select = function(evt) {
            ...
            this.result_clear_highlight(),
            this.is_multiple ? this.result_deactivate(high) : (this.search_results.find(".result-selected").removeClass("result-selected"), this.result_single_selected = high, this.selected_item.removeClass("chzn-default")),
            high.addClass("result-selected"),
            position = high_id.substr(high_id.lastIndexOf("_") + 1),
            item = this.results_data[position],
            item.selected = !0,
            this.form_field.options[item.options_index].selected = !0,
            this.is_multiple ? this.choice_build(item) : (this.selected_item.find("span").first().text(item.text), this.allow_single_deselect && this.single_deselect_control_build()),
            (!evt.metaKey || !this.is_multiple) && this.results_hide(),
        ...
        Chosen.prototype.winnow_results = function() {
            ...
            for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                ...
                if (!option.disabled && !option.empty) if (option.group) $("#" + option.dom_id).css("display", "none");
                    ...
                    result = $("#" + result_id);
                    //换成整个内容模糊查询，默认只匹配第一个字符
                    if (zregex.test(option.html)) found = !0,
                    results += 1;
                    else if (option.html.indexOf(" ") >= 0 || option.html.indexOf("[") === 0) {
                        parts = option.html.replace(/\[|\]/g, "").split(" ");
                        if (parts.length) for (_j = 0, _len2 = parts.length; _j < _len2; _j++) part = parts[_j],
                        zregex.test(part) && (found = !0, results += 1)
                    }
        ...
        Chosen.prototype.winnow_results_clear = function() {
            ...
            _results = [];
            for (_i = 0, _len = lis.length; _i < _len; _i++) li = lis[_i],
            li = $(li),
            li.hasClass("group-result") ? _results.push(li.css("display", "auto")) : !this.is_multiple || !li.hasClass("result-selected") ? _results.push(this.result_activate(li)) : _results.push(void 0);
            return _results
        },
        ...
        Chosen.prototype.no_results = function(terms) {
        	/*
        	//获得其对应的select的name
        	var selectName = this.search_results.parent().parent().prev().attr("name");
        	//定义新输入的内容，即要向ul追加的li
        	var no_results_html;
        	//获得要追加的li的id的前半部分
        	var changeId = $(this).attr("container_id");
        	//获得以changeId开头的li的个数
        	var liLength = $("li[id*="+changeId+"]").length;
        	//var liLength2 = Number(liLength)-1;
        	//获得下拉列表的id
            var selecetId = changeId.substr(0,changeId.lastIndexOf('_'));
            //alert(selecetId);
            //获得该下拉列表的option的个数
            //var selectLength = $("#"+selecetId).children().length;
            //获得规格值主键id
           // var keyid = makeuuid();
            //chosen_count++;
            //获得当前的规格id
            var specification_id = $("#"+selecetId).parents("div.sku-sub-group").find("h3 select").val();
        	if(selectName=="specifications_value"){
                //新输入的内容，即要向ul追加的li
            	return no_results_html = $('<li name="extraLi" id="'+changeId+'_o_'+liLength+'" class="active-result" id="'+changeId+'_o_'+liLength+'" >' + terms + '</li>'),
            	//删除之前追加的li
                this.search_results.find("li[name='extraLi']").remove(),
                //删除之前追加的option
                $("#"+selecetId).find("option[class='extraOption']").remove(),
                //执行后台新增方法
                //addGuigeValOption(keyid,terms,specification_id),
                //addGuigeValOption(chosen_count,terms,specification_id,chosen_count-1),
                //向其对应的select列表中添加option
            	//$("#"+selecetId).append('<option value="'+keyid+'" class="extraOption">'+terms+'</option>'),
            	$("#"+selecetId).append('<option value="'+chosen_count+'" class="extraOption">'+terms+'</option>'),
            	//在ul中追加li
                this.search_results.append(no_results_html);
        	}else if(selectName=="group_id" || selectName=="specifications_id"){*/
        		return no_results_html = $('<li class="no-results">' + this.results_none_found + ' "<span></span>"</li>'),
                no_results_html.find("span").first().html(terms),
                this.search_results.append(no_results_html)
        	//}
        	/*
        	else if(selectName=="specifications_id"){
        		return no_results_html = $('<li name="extraLi" id="'+changeId+'_o_'+liLength+'" class="active-result" id="'+changeId+'_o_'+liLength+'" >' + terms + '</li>'),
            	//删除之前追加的li
                this.search_results.find("li[name='extraLi']").remove(),
                //删除之前追加的option
                $("#"+selecetId).find("option[class='extraOption']").remove(),
                //执行后台新增方法
                addGuigeOption(keyid,terms,specification_id),
                //向其对应的select列表中添加option
            	$("#"+selecetId).append('<option value="'+keyid+'" class="extraOption">'+terms+'</option>'),
            	//在ul中追加li
                this.search_results.append(no_results_html);
        ...
        Chosen.prototype.keyup_arrow = function() {
            ...
            if (this.result_highlight) return prev_sibs = this.result_highlight.prevAll("li.active-result"),
            prev_sibs.length ? this.result_do_highlight(prev_sibs.first()) : (this.choices > 0 && this.results_hide(), this.result_clear_highlight())
        },
        ...
        Chosen.prototype.keydown_checker = function(evt) {
            ...
            stroke = (_ref = evt.which) != null ? _ref: evt.keyCode,
            this.search_field_scale(),
            stroke !== 8 && this.pending_backstroke && this.clear_backstroke();
            switch (stroke) {
            case 8:
        ...
        Chosen.prototype.search_field_scale = function() {
            ...
            if (this.is_multiple) {
                ...
                for (_i = 0, _len = styles.length; _i < _len; _i++) style = styles[_i],
                ...
                w = div.width() + 25,
                div.remove(),
                w > this.f_width - 10 && (w = this.f_width - 10),
...
//规格字典表中新增规格
function addGuigeOption(keyid,specification_name){
	$.ajax({
        url : getRootPath()+"/commodity/addCommoditySpecifications.action",
		type : "post",
        data : {specification_name:specification_name,keyid:keyid},
        dataType:"json",
        async:false,
...
//规格值字典表中新增规格值
function addGuigeValOption(keyid,specification_value_name,specification_id,keyid_1){
	$.ajax({
        url : getRootPath()+"/commodity/addCommoditySpecificationsVal.action",
		type : "post",
        data : {specification_id:specification_id,specification_value_name:specification_value_name,keyid:keyid,keyid_1:keyid_1},
        dataType:"json",
        async:false,
        success : function (result){
