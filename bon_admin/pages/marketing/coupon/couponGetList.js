	...
	var queryCond = $('#queryform').serializeObject();
	 ...
	 group = $('#group_list').DataTable({
	        ...
	        "columnDefs": [
	                     ...
	                     "ajax": {
	                    	 "type":"post",
	                         "url": getRootPath()+"/coupon/queryCouponGetPageList.action",
	                         "dataType":"json",
...
//组装订单详情列
function couponOrderDetail(full){
	var str = "";
...
//组装状态
function couponStatus(full){
	var str = "";
	if(full.to_use == 1){
		str = '已使用';
	}else if(full.to_use == 0){
		str = '未使用';
	}
	return str;
}
