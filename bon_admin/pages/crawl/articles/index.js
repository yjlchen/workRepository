    ...
    var bon_crawl_account_id = theRequest.id;
	 ...
	 var cust = $('#mission_list').dataTable({
	        ...
	        "columnDefs": [
	                     ...
	                     "ajax": {
	                         "url": getRootPath()+"/crawl/queryArticlePageList.action",
	                         "type":"post",
	                         "dataType":"json",
	                         'data' : {
	                        	 "queryJsonStr":queryJsonStr,
								 "bon_crawl_account_id": bon_crawl_account_id,
	                         }
	                     },
	                     "language": {
...
function queryLabel(id){	
	...
	var showX = (width/2);
	window.open('article.jsp?id='+id,'_blank','width=600,height='+height+',top=0px,left='+showX+'px')
}